...
public class SysDicController {
    ...
    public IPage<SysDic> list(int currentPage, int pageSize, String flag, String name) {
        QueryWrapper<SysDic> queryWrapper = new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).eq("org_id",GlobalParam.orgId);
    ...
    public Map<String,String> getAppRoleMap(String status, String flag, String name) {
        ...
        List<SysDic> list =  sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).eq("status","角色"));
    ...
    public Map<String,Object> getReportPathMap(String status, String flag, String name) {
        ...
        List<SysDic> list;
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).eq("flag","资产概览").eq("status","报表"));
        if(CollUtil.isNotEmpty(list))
            map.put("资产概览",list.get(0).getName());
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).eq("flag","指标监控").eq("status","报表"));
        if(CollUtil.isNotEmpty(list))
            map.put("指标监控",list.get(0).getName());
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).eq("flag","报表路径前缀").eq("status","报表"));
        if(CollUtil.isNotEmpty(list))
            map.put("报表路径前缀",list.get(0).getName());
        Map<String,String> map_process = new HashMap<>();
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).like("flag","流程").eq("status","报表"));
        if(CollUtil.isNotEmpty(list)){
            for(SysDic s :list){
        ...
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).like("flag","生命周期").eq("status","报表"));
        if(CollUtil.isNotEmpty(list)){
            map.put("生命周期",list.get(0).getName());
        }
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).like("flag","生命周期_查询").eq("status","报表"));
        if(CollUtil.isNotEmpty(list)){
            map.put("生命周期_查询",list.get(0).getName());
        }
        list = sysDicService.list(new  QueryWrapper<SysDic>().eq("org_id",GlobalParam.orgId).like("flag","资产台账").eq("status","报表"));
        if(CollUtil.isNotEmpty(list)){
