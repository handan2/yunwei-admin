...
public class OperateLogAspect {
   ...
   //环绕通知：暂不深研
    ...
    private void saveOperateLog(ProceedingJoinPoint joinPoint, int time) {
        ...
        } else if (operateLog.type().contains("登陆")) {//20240220
        ...
        if(ObjectUtil.isEmpty(ip)){
            ip = ServletUtil.getClientIP(httpServletRequest);//20240219这个httpServletRequest的生命周期：网关登陆时，他的在线状态是由后台进程维持：即使关了网关和浏览器也不会立即中止他
