...
public class ExcelDateUtil {
            ...
            for (T obj : dataList) {
                for (String dateName : dateNameList) {
                    ...
                    if (ObjectUtil.isNotEmpty(dateValue)) {
                        ...
                        //字符串转为LocalDateTime
                        //yyyy-MM-dd HH:mm:ss
                        String regex1 = "\\d{4}[-]\\d{2}[-]\\d{1,2} \\d{2}:\\d{2}:\\d{2}";//20230529 月日都从d{2}改成d{1,2}；20240628又改回d{2}，因为 LocalDate.parse里的规则不支持单月/日
                        //yyyy-MM-dd
                        String regex2 = "\\d{4}[-]\\d{2}[-]\\d{1,2}";
                        //yyyy/MM/dd
                        String regex3 = "\\d{4}[/]\\d{2}[/]\\d{1,2}";
                        ...
                        if (ReUtil.isMatch(regex1, dateValue)) { //20221115 两类日期格式都统一转成了短日期
                            ...
                            //20221215 注意 excel里如果是字符串类型&&2022/12/1这种也报错：必须是写全了“2022/12/01”:待后续研
                            DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
                            ReflectUtil.setFieldValue(obj, dateName.replaceAll("Tmp", ""), LocalDate.parse(dateValue, dateTimeFormatter));
                        } else {
                            throw new RuntimeException(dateValue + "格式错误：请确保原单元格是日期格式！");
                        }
                    }
                }
            }
    ...
    public static <T,V> void dateConverToString(T  dbData, V excelData,Class clazz) {
        //获取buyDateTmp格式的日期字段名称
        List<String> dateNameList = new ArrayList<>();
        List<String> dateNameList1 = new ArrayList<>();
        Field[] fieldArr = ReflectUtil.getFields(clazz);
        for (Field field : fieldArr) {
        ...
        for (Field field : fieldArr) {//20230815添加
            if (field.getName().endsWith("Datetime")) {
                dateNameList1.add(field.getName());
            }
        }
        //
        if (ObjectUtil.isNotEmpty(dbData)) {
            for (String dateName : dateNameList) {
                ...
                if (ObjectUtil.isNotEmpty(dateValue)) {
                    ...
                    if(ObjectUtil.isNotEmpty(ReflectUtil.getField(excelData.getClass(),dateName+"Tmp")))//20230529
                        ReflectUtil.setFieldValue(excelData, dateName+"Tmp", dateStr);
                }
            }
            for (String dateName : dateNameList1) {
                LocalDateTime dateValue = (LocalDateTime) ReflectUtil.getFieldValue(dbData, dateName);
                if (ObjectUtil.isNotEmpty(dateValue)) {
