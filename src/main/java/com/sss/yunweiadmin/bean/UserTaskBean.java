...
public class UserTaskBean {
    ...
    public List<SysUser> getUserList(Integer processDefinitionId, String preTaskId, String currentTaskId,Integer processInstanceDataId) {
        ...
        if (preTask.getHaveNextUser().equals("是")) {
            ...
            return getUserList(nextUserVO.getType(), nextUserVO.getTypeValue(), nextUserVO.getHaveStarterDept(),processInstanceDataId);
        } else {
            return getUserList(currentTask.getType(), currentTask.getTypeValue(), currentTask.getHaveStarterDept(),processInstanceDataId);
        }
    }
//todo改造：这个函数及上面那个都需要添加一个流程实例ID的参数
    //listener里调用他，实参要加
    public List<SysUser> getUserList(String type, String typeValue, String haveStarterDept,Integer processInstanceDataId) {
