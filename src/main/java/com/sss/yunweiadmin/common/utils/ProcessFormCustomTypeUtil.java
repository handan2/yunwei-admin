...
//                tableNameList.add(entry.getKey());
//            }
            //20220720 下面那个&&也有依次执行，遇第一个为false不执行的机制
            if (ObjectUtil.isNotEmpty(entry.getValue()) && entry.getValue().equals("是")) {//20220718逻辑有问题：这里限制了只能选自定义表实例中已选字段对应的基本表：实际应该是所有‘flag=是’的基本表（不管有没有选过当自定义表字段）
