...
public class ProcessFormTemplateServiceImpl extends ServiceImpl<ProcessFormTemplateMapper, ProcessFormTemplate> implements ProcessFormTemplateService {
    ...
    public Map<String, String> getLabelIdMapForItemByAjax(Integer processDefinitionId, String hideGroupIds) {
        ...
        //20231116 又取消注释了：因为会根据“隐藏字段组不在前端渲染DOM”的机制来控制一些前端逻辑
        ...
        for (ProcessFormTemplate processFormTemplate : list) {
            ...
            if ("字段变更类型".equals(processFormTemplate.getFlag())) {
                if(processDefinition.getProcessName().contains("密钥更换")){//20230706 此流程有两个(来自不同基本表的)同名变更字段
                    String changeColumnName = processFormTemplate.getLabel().split("\\.")[1];
                    if(changeColumnName.equals("责任人") || changeColumnName.equals("责任部门") || changeColumnName.equals("责任人密级")){
                        if(processFormTemplate.getLabel().contains("密钥信息表2")){
                            map.put(changeColumnName, processFormTemplate.getId().toString());
                        }
                    }
                } else
...
//        }
        ...
        if(processDefinition.getStartLimitedByCheck().equals("是") && ObjectUtil.isNotEmpty(actProcessInstanceId) &&  !actProcessInstanceId.equals("-1") ){
            ...
            //读取并组装开关变量
            //对于授权发起流程：再遍历一遍，给相应formitem赋初值；20230223这段代码注释掉（因为维修流程里不引用前置流程的开关量了）：20230530先解开
            for (Map.Entry<Integer, FormTemplateVO> entry : map.entrySet()) {
                ...
                } else if(entry.getValue().getLabel().contains("硬件维修") && !entry.getValue().getLabel().contains(".")  ) {//20230530 contains("硬件维修")和 "硬件维修信息表.xxx"这种格式的变更字段重名
                    String itemId = labelIdMapForItemByAjax.get("硬件维修");
                    if(ObjectUtil.isNotEmpty(itemId)){
                        entry.getValue().setDefaultValue(jsonObject.getString(itemId));
                    }
                }
